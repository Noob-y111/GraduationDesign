...
//        dialog?.window?.setWindowAnimations(R.style.player_dialog_fragment)
    ...
    protected fun translateStatusBarBlackBackground(){
        val window = dialog?.window
        window?.let {
            val point = Point()
            it.windowManager?.defaultDisplay?.getSize(point)
    ...
    protected fun dialogWidthAndHeight(@IntRange(from = 0, to = 100) wPercent: Int, @IntRange(from = 0, to = 100) hPercent: Int){
        val window = dialog?.window
        window?.let {
            val point = Point()
            it.windowManager?.defaultDisplay?.getSize(point)
            it.attributes.height = (point.y * (hPercent.toFloat()/100)).toInt()
            it.attributes.width = (point.x * (wPercent.toFloat()/100)).toInt()
            it.setGravity(Gravity.BOTTOM)
            it.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
            it.decorView.systemUiVisibility = View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN or View.SYSTEM_UI_FLAG_LAYOUT_STABLE
            it.statusBarColor = Color.TRANSPARENT
